...
for idx, run in enumerate(range(runs)): 
    for size in sizes:
        ...
        tensor3d = tf.random.normal(shape=(size, size, size))
        #print(tensor3d)
        ...
        with contextlib.redirect_stdout(text_stream):
            ...
            T, orig_shape = dc.from_tensorflow(tensor3d, return_shape=True)
            tensor3d = T.max(T).compute(isTensorflow=True, verbose=True, shape=orig_shape)
        ...
        data = [num_dim1, num_dim2, num_dim3, exec_func_exec_time, tensor_trans_exec_time, overall_comp_exec_time, tensor_size_mb]

...
void checkInsertRow(const DTArg *arg, const DTArg *ins, const VTSel lowerIncl,
                    const VTSel upperExcl, const DTArg *exp) {
    DTArg *res = nullptr;
    insertRow<DTArg, DTArg, VTSel>(res, arg, ins, lowerIncl, upperExcl,
                                   nullptr);
    CHECK(*res == *exp);
...
template <typename DTArg, typename VTSel>
void checkInsertRowThrow(const DTArg *arg, const DTArg *ins,
                         const VTSel lowerIncl, const VTSel upperExcl) {
    DTArg *res = nullptr;
    REQUIRE_THROWS_AS((insertRow<DTArg, DTArg, VTSel>(res, arg, ins, lowerIncl,
                                                      upperExcl, nullptr)),
                      std::out_of_range);
}

TEMPLATE_PRODUCT_TEST_CASE("InsertRow", TAG_KERNELS, (DATA_TYPES),
                           (VALUE_TYPES)) {
    using DT = TestType;
...
TEMPLATE_PRODUCT_TEST_CASE("InsertRow - FP specific", TAG_KERNELS, (DATA_TYPES),
                           (double)) {
    using DT = TestType;
    using VT = typename DT::VT;

...
TEMPLATE_PRODUCT_TEST_CASE("MatMulCUDA", TAG_KERNELS, (DenseMatrix), (float, double)) {
    ...
    user_config.cuda_devices.push_back(0);
    CUDAContext* ctx = CUDAContext::create(&user_config);
    ...
    checkMatMulCUDA(m0, m0, m0, *ctx);
    checkMatMulCUDA(m1, m1, m2, *ctx);
    checkMatMulCUDA(m3, m4, m5, *ctx);
    checkMatMulCUDA(m0, v0, v0, *ctx);
    checkMatMulCUDA(m1, v0, v0, *ctx);
    checkMatMulCUDA(m2, v0, v0, *ctx);
    checkMatMulCUDA(m0, v1, v0, *ctx);
    checkMatMulCUDA(m1, v1, v3, *ctx);
    checkMatMulCUDA(m1, v2, v4, *ctx);

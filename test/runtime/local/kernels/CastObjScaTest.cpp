...
TEMPLATE_TEST_CASE("castObjSca, matrix to scalar", TAG_KERNELS, double, float,
    ...
    SECTION("DenseMatrix<int64_t> to VTRes") {
        VTRes exp = 2;
        auto m0 =
            genGivenVals<DenseMatrix<int64_t>>(1, {static_cast<int64_t>(exp)});
    ...
    SECTION("DenseMatrix<double> to VTRes") {
        VTRes exp = 2.2;
        auto m0 =
            genGivenVals<DenseMatrix<double>>(1, {static_cast<double>(exp)});
...
TEMPLATE_TEST_CASE("castObjSca, frame to scalar", TAG_KERNELS, double, float,
                   int64_t, uint64_t, int32_t, uint32_t) {
    using VTRes = TestType;

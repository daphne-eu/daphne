...
TEMPLATE_TEST_CASE("Tensor Creation", TAG_DATASTRUCTURES, double, float,
    ...
    ContiguousTensor<TestType> *ct0 =
        DataObjectFactory::create<ContiguousTensor<TestType>>(rank0_shape,
                                                              InitCode::IOTA);
    ContiguousTensor<TestType> *ct1 =
        DataObjectFactory::create<ContiguousTensor<TestType>>(rank1_shape,
                                                              InitCode::IOTA);
    ContiguousTensor<TestType> *ct2 =
        DataObjectFactory::create<ContiguousTensor<TestType>>(rank2_shape,
                                                              InitCode::IOTA);
    ContiguousTensor<TestType> *ct3 =
        DataObjectFactory::create<ContiguousTensor<TestType>>(rank3_shape,
                                                              InitCode::IOTA);
    ContiguousTensor<TestType> *ct4 =
        DataObjectFactory::create<ContiguousTensor<TestType>>(rank4_shape,
                                                              InitCode::IOTA);

    ChunkedTensor<TestType> *cht0 =
        DataObjectFactory::create<ChunkedTensor<TestType>>(
            rank0_shape, rank0_chunk_shape, InitCode::IOTA);
    ChunkedTensor<TestType> *cht1 =
        DataObjectFactory::create<ChunkedTensor<TestType>>(
            rank1_shape, rank1_chunk_shape, InitCode::IOTA);
    ChunkedTensor<TestType> *cht2 =
        DataObjectFactory::create<ChunkedTensor<TestType>>(
            rank2_shape, rank2_chunk_shape, InitCode::IOTA);
    ChunkedTensor<TestType> *cht3 =
        DataObjectFactory::create<ChunkedTensor<TestType>>(
            rank3_shape, rank3_chunk_shape, InitCode::IOTA);
    ChunkedTensor<TestType> *cht4 =
        DataObjectFactory::create<ChunkedTensor<TestType>>(
            rank4_shape, rank4_chunk_shape, InitCode::IOTA);
...
TEMPLATE_TEST_CASE("Tensor layout and accessors", TAG_DATASTRUCTURES, double,
                   float, uint32_t, uint64_t, int32_t, int64_t) {
    ...
    ChunkedTensor<TestType> *cht0 =
        DataObjectFactory::create<ChunkedTensor<TestType>>(
            tensor_shape0, chunk_shape0, InitCode::IOTA);
    ChunkedTensor<TestType> *cht1 =
        DataObjectFactory::create<ChunkedTensor<TestType>>(
            tensor_shape1, chunk_shape1, InitCode::IOTA);
        ...
        for (size_t i = 0; i < 4; i++) {
            for (size_t j = 0; j < 4; j++) {
                for (size_t k = 0; k < 4; k++) {
                    ...
                    REQUIRE(ct0->get(ids) ==
                            getIOTAValue<TestType>(tensor_shape0, ids));
                    REQUIRE(cht0->get(ids) ==
                            getIOTAValue<TestType>(tensor_shape0, ids));
                }
            }
        }
            ...
            for (size_t j = 0; j < 2; j++) {
                for (size_t k = 0; k < 7; k++) {
                    for (size_t n = 0; n < 5; n++) {
                        ...
                        REQUIRE(ct1->get(ids) ==
                                getIOTAValue<TestType>(tensor_shape1, ids));
                        REQUIRE(cht1->get(ids) ==
                                getIOTAValue<TestType>(tensor_shape1, ids));
                    }
                }
            }
        ...
        for (size_t i = 0; i < 4; i++) {
            for (size_t j = 0; j < 4; j++) {
                for (size_t k = 0; k < 4; k++) {
                    ...
                    REQUIRE(cht0->get(ids) ==
                            static_cast<TestType>(test_value));
...
TEMPLATE_TEST_CASE("Tensor dicing, rechunking and conversions",
                   TAG_DATASTRUCTURES, double, float, uint32_t, uint64_t,
                   int32_t, int64_t) {
    ...
    ChunkedTensor<TestType> *cht =
        DataObjectFactory::create<ChunkedTensor<TestType>>(
            tensor_shape, chunk_shape, InitCode::IOTA);

    SECTION(".dice() and variants") {
        std::vector<std::pair<size_t, size_t>> dice_range = {
            {1, 3}, {3, 4}, {0, 2}};
    ...
    SECTION("Converion: rank 2 ContiguousTensor -> DenseMatrix") {
        ContiguousTensor<TestType> *ct1 =
            DataObjectFactory::create<ContiguousTensor<TestType>>(
                std::vector<size_t>({4, 3}), InitCode::IOTA);
        ContiguousTensor<TestType> *ct2 =
            DataObjectFactory::create<ContiguousTensor<TestType>>(
                std::vector<size_t>({4, 3, 3}), InitCode::IOTA);

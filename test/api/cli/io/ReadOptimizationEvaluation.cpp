...
std::string createDaphneScript(const std::string &evaluationDir, const std::string &csvFilename,
...
template <typename... Args> std::string runDaphneEval(const std::string &scriptFilePath, Args... args) {
...
//   read time: 117784ns
//   {"startup_seconds": 0.0136333, "parsing_seconds": 0.000770869, "compilation_seconds": 0.0182154,
//   "execution_seconds": 0.00726858, "total_seconds": 0.0398881}
...
void writeResultsToFile(const std::string &feature, const std::string &csvFilename, bool opt, bool firstRead,
    ...
    if (!fileExists) {
        ofs << "CSVFile,OptEnabled,FirstRead,NumCols,NumRows,FileType,ReadTime,WriteTime,StartupSeconds,ParsingSeconds,"
               "CompilationSeconds,ExecutionSeconds,TotalSeconds,WriteTime\n";
    }
    ...
    ofs << csvFilename << "," << optStr << "," << firstReadStr << "," << numCols << "," << numRows << "," << type << ","
        << timingData.readTime << "," << timingData.writeTime << "," << timingData.startupSeconds << ","
        << timingData.parsingSeconds << "," << timingData.compilationSeconds << "," << timingData.executionSeconds
        << "," << timingData.totalSeconds << "\n";
    ofs.close();
}

void runEvalTestCase(const std::string &csvFilename, std::string feature = "posmap", std::string daphneScript = "",
    ...
    output = runDaphneEval(daphneScript, "--timing", "--use-positional-map");
    std::cout << output << std::endl;
...
TEST_CASE("EvalTestFrameNumber60KB", TAG_IO) {
...
TEST_CASE("EvalTestFrameNumber6MB", TAG_IO) {
    // Example instantiation.
...
TEST_CASE("EvalStrMatrix", TAG_IO) {
    // Example instantiation.
    const std::string csvFilename = "data_100r_10c_FIXEDSTR.csv";
    const std::string daphneScript = "evalReadStr.daphne";
    runEvalTestCase(csvFilename, "posmap"); //, daphneScript);
}

TEST_CASE("EvalStrMatrix2", TAG_IO) {
    // Example instantiation.
    const std::string csvFilename = "data_1000r_100c_FIXEDSTR.csv";
    const std::string daphneScript = "evalReadStr2.daphne";
    runEvalTestCase(csvFilename, "posmap"); //, daphneScript);
}
TEST_CASE("EvalStrMatrix3", TAG_IO) {
    // Example instantiation.
    const std::string csvFilename = "data_10000r_1000c_FIXEDSTR.csv";
    const std::string daphneScript = "evalReadStr3.daphne";
    runEvalTestCase(csvFilename, "posmap"); //, daphneScript);
}

TEST_CASE("EvalNumberMatrix", TAG_IO) {
    // Example instantiation.
    const std::string csvFilename = "data_1000r_1000c_NUMBER.csv";
    const std::string daphneScript = "evalReadMatrix.daphne";
    runEvalTestCase(csvFilename, "posmap", daphneScript);
}

TEST_CASE("EvalTestFrameStr200MB", TAG_IO) {
    // Example instantiation.
    const std::string csvFilename = "data_10000r_1000c_FIXEDSTR.csv";
    const std::string daphneScript = "evalReadFrame3.daphne";
    runEvalTestCase(csvFilename, "posmap", daphneScript);
}
...
void runEvalTestCase(const std::string &csvFilename, std::string daphneScript = "",
    ...
    output = runDaphneEval(daphneScript, "--timing", "--use-dbdf-optimization");
    std::cout << output << std::endl;
...
TEST_CASE("evalFrameFromCSVBinOpt", TAG_IO) {
    ...
    // build binary file and positional map on first read
    output = runDaphneEval(dirPath + "evalReadFrame.daphne", "--timing", "--use-dbdf-optimization");
    std::cout << output << std::endl;
    timingData = extractTiming(output, true);

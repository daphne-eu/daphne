...
for csvfile in "$CSV_DIR"/*.csv; do 
    ...
    # Header: CSVFile,Experiment,Trial,ReadTime,WriteTime,StartupSeconds,ParsingSeconds,CompilationSeconds,ExecutionSeconds,TotalSeconds
    for logfile in "$logNormal" "$logCreate" "$logOpt"; do
        if [ ! -f "$logfile" ]; then
            echo "CSVFile,Experiment,Trial,ReadTime,WriteTime,StartupSeconds,ParsingSeconds,CompilationSeconds,ExecutionSeconds,TotalSeconds" > "$logfile"
        fi
    done
    ...
    for i in $(seq 1 $((REPS+1))); do
         ...
         write_line=$(echo "$output" | grep "OPERATION=write_bin," | head -n1)

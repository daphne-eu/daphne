...
for csv_file in csv_paths:
    ...
    # The corresponding posmap file is assumed to be named as the CSV plus a ".posmap" extension.
    posmap_file = csv_file + ".posmap"
    if os.path.exists(posmap_file):
        posmap_size = os.path.getsize(posmap_file)
    else:
        posmap_size = None
    ...
    ratio = (posmap_size / csv_size * 100) if posmap_size is not None else None
...
print("\nAverage percentage of Posmap size relative to CSV size by main data type:")
print(avg_main)

# Compute the average ratio per DataSubtype.
avg_subtype = df.groupby("DataSubtype")["Ratio (%)"].mean().reset_index()
print("\nAverage percentage of Posmap size relative to CSV size by data subtype:")
print(avg_subtype)
...
plt.ylabel("Average Posmap/CSV Size Ratio (%)")
plt.title("Comparison: 100% CSV vs. Average Posmap Ratios by Data Type/Subtype")

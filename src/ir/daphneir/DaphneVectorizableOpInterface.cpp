...
std::vector<daphne::VectorSplit> getVectorSplits_EwBinaryOp(EwBinaryOp *op) {
    ...
    auto lhsSplit = op->getLhs().getType().template isa<daphne::MatrixType>()
                        ? daphne::VectorSplit::ROWS
                        : daphne::VectorSplit::NONE;
    auto rhsSplit = op->getRhs().getType().template isa<daphne::MatrixType>()
                        ? daphne::VectorSplit::ROWS
                        : daphne::VectorSplit::NONE;
    return {lhsSplit, rhsSplit};
}
template <class EwBinaryOp>
std::vector<daphne::VectorCombine>
...
template <class RowAggOp>
std::vector<std::pair<Value, Value>>
createOpsOutputSizes_RowAggOp(RowAggOp *op, OpBuilder &builder) {
    auto loc = op->getLoc();
    auto sizeTy = builder.getIndexType();
    auto rows = builder.create<daphne::NumRowsOp>(loc, sizeTy, op->getArg());
    auto cst1 = builder.create<daphne::ConstantOp>(loc, sizeTy,
...
template <class ColAggOp>
std::vector<std::pair<Value, Value>>
createOpsOutputSizes_ColAggOp(ColAggOp *op, OpBuilder &builder) {
    auto loc = op->getLoc();
    auto sizeTy = builder.getIndexType();
    auto cst1 = builder.create<daphne::ConstantOp>(loc, sizeTy,
                                                   builder.getIndexAttr(1l));
...
#define IMPL_SPLIT_COMBINE_EWBINARYOP(OP)                                      \
    ...
    std::vector<std::pair<Value, Value>> daphne::OP::createOpsOutputSizes(     \
        OpBuilder &builder) {                                                  \

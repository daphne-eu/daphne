...
struct DaphneInlinerInterface : public mlir::DialectInlinerInterface {
    ...
    void
    handleTerminator(mlir::Operation *op,
        ...
        if (returnOp.getNumOperands() != valuesToRepl.size()) {
            throw ErrorHandler::compilerError(
                op, "DaphneInlinerInterface (handleTerminator)",
                "number of operands " +
                    std::to_string(returnOp.getNumOperands()) + " from " +
                    op->getName().getStringRef().str() + " do not match size " +
                    std::to_string(valuesToRepl.size()));
        }
    ...
    materializeCallConversion(mlir::OpBuilder &builder, mlir::Value input,
        ...
        return builder.create<mlir::daphne::CastOp>(conversionLoc, resultType,
                                                    input);
    }
};
...
mlir::Operation *mlir::daphne::DaphneDialect::materializeConstant(
    OpBuilder &builder, Attribute value, Type type, mlir::Location loc) {
    return builder.create<mlir::daphne::ConstantOp>(loc, type, value);
}

mlir::Type
mlir::daphne::DaphneDialect::parseType(mlir::DialectAsmParser &parser) const {
    ...
    if (keyword == "Matrix") {
        ...
        MatrixRepresentation representation =
            MatrixRepresentation::Default; // default is dense
        ...
        while (succeeded(parser.parseOptionalColon())) {
            if (succeeded(parser.parseOptionalKeyword("sp"))) {
                ...
                if (parser.parseLSquare() || parser.parseFloat(sparsity) ||
                    parser.parseRSquare()) {
                    return nullptr;
                }
            } else if (succeeded(parser.parseOptionalKeyword("rep"))) {
                llvm::StringRef repName;
                if (parser.parseLSquare() || parser.parseKeyword(&repName) ||
                    parser.parseRSquare()) {
                    return nullptr;
                }
        ...
        return MatrixType::get(parser.getBuilder().getContext(), elementType,
                               numRows, numCols, sparsity, representation);
        ...
        if (parser.parseLess() || parser.parseOptionalQuestion() ||
            ...
            parser.parseKeyword("x") || parser.parseLSquare() ||
        ...
        return FrameType::get(parser.getBuilder().getContext(), cts, numRows,
                              numCols, nullptr);
    } else if (keyword == "Handle") {
        mlir::Type dataType;
        if (parser.parseLess() || parser.parseType(dataType) ||
            parser.parseGreater()) {
            return nullptr;
        }
        return mlir::daphne::HandleType::get(parser.getBuilder().getContext(),
                                             dataType);
    } else if (keyword == "String") {
        return StringType::get(parser.getBuilder().getContext());
    } else if (keyword == "DaphneContext") {
        return mlir::daphne::DaphneContextType::get(
            parser.getBuilder().getContext());
    } else {
        parser.emitError(parser.getCurrentLocation())
            << "Parsing failed, keyword `" << keyword << "` not recognized!";
        return nullptr;
    }
}
...
void mlir::daphne::DaphneDialect::printType(mlir::Type type,
    ...
    if (type.isa<mlir::daphne::StructureType>())
        ...
        os << "Matrix<" << unknownStrIf(t.getNumRows()) << 'x'
           << unknownStrIf(t.getNumCols()) << 'x' << t.getElementType();
        ...
        os << "Frame<" << unknownStrIf(t.getNumRows()) << "x["
...
std::string
mlir::daphne::matrixRepresentationToString(MatrixRepresentation rep) {
    switch (rep) {
...
mlir::daphne::MatrixRepresentation
mlir::daphne::stringToMatrixRepresentation(const std::string &str) {
    if (str == "dense")
        ...
        throw std::runtime_error(
            "No matrix representation equals the string `" + str + "`");
}
...
struct MatrixTypeStorage : public ::mlir::TypeStorage {
    ...
    MatrixTypeStorage(::mlir::Type elementType, ssize_t numRows,
                      ssize_t numCols, double sparsity,
                      MatrixRepresentation representation)
        : elementType(elementType), numRows(numRows), numCols(numCols),
          sparsity(sparsity), representation(representation) {}
    ...
    static ::llvm::hash_code hashKey(const KeyTy &tblgenKey) {
        auto float_hashable =
            static_cast<ssize_t>(std::get<3>(tblgenKey) / epsilon);
        return ::llvm::hash_combine(
            std::get<0>(tblgenKey), std::get<1>(tblgenKey),
            std::get<2>(tblgenKey), float_hashable, std::get<4>(tblgenKey));
    }
    ...
    static MatrixTypeStorage *construct(::mlir::TypeStorageAllocator &allocator,
        ...
        return new (allocator.allocate<MatrixTypeStorage>()) MatrixTypeStorage(
            elementType, numRows, numCols, sparsity, representation);
    }
    ::mlir::Type elementType;
    ssize_t numRows;
    ssize_t numCols;
...
::mlir::Type MatrixType::getElementType() const {
...
MatrixRepresentation MatrixType::getRepresentation() const {
    return getImpl()->representation;
}
} // namespace mlir::daphne
...
mlir::OpFoldResult mlir::daphne::ConstantOp::fold(FoldAdaptor adaptor) {
    if (!adaptor.getOperands().empty())
        throw ErrorHandler::compilerError(
            this->getLoc(),
            "CanonicalizerPass (mlir::daphne::ConstantOp::fold)",
...
::mlir::LogicalResult mlir::daphne::MatrixType::verify(
    ...
    Type elementType, ssize_t numRows, ssize_t numCols, double sparsity,
    ...
    if ((
            ...
            elementType.isUnsignedInteger(64) || elementType.isF32() ||
            elementType.isF64() || elementType.isIndex() ||
...
::mlir::LogicalResult mlir::daphne::HandleType::verify(
    ::llvm::function_ref<::mlir::InFlightDiagnostic()> emitError,
    Type dataType) {
    if (llvm::isa<MatrixType>(dataType)) {
        ...
        return emitError()
               << "only matrix type is supported for handle atm, got: "
               << dataType;
}
...
mlir::Attribute performCast(mlir::Attribute attr, mlir::Type targetType,
                            mlir::Location loc);
...
template <class ArgAttrElementT, class ResAttrElementT = ArgAttrElementT,
          ...
          class CalculationT = std::function<ResElementValueT(
              const ArgElementValueT &, const ArgElementValueT &)>>
mlir::Attribute constFoldBinaryOp(mlir::Location loc, mlir::Type resultType,
                                  llvm::ArrayRef<mlir::Attribute> operands,
                                  const CalculationT &calculate) {
    if (operands.size() != 2)
        throw ErrorHandler::compilerError(
            loc, "CanonicalizerPass (constFoldBinaryOp)",
            "binary op takes two operands but " +
                std::to_string(operands.size()) + " were given");

    if (!operands[0] || !operands[1])
        return {};

    if (llvm::isa<ArgAttrElementT>(operands[0]) &&
        llvm::isa<ArgAttrElementT>(operands[1])) {
        ...
        if constexpr (std::is_same<ResAttrElementT, mlir::IntegerAttr>::value ||
            ...
            if ((l.dyn_cast<mlir::IntegerType>() ||
                 l.dyn_cast<mlir::FloatType>()) &&
                (r.dyn_cast<mlir::IntegerType>() ||
                 r.dyn_cast<mlir::FloatType>())) {
                ...
                if (lhsBitWidth < rhsBitWidth) {
                    mlir::Attribute promotedLhs =
                        performCast(lhs, rhs.getType(), loc);
                    lhs = promotedLhs.cast<ArgAttrElementT>();
                } else if (rhsBitWidth < lhsBitWidth) {
                    mlir::Attribute promotedRhs =
                        performCast(rhs, lhs.getType(), loc);
                    rhs = promotedRhs.cast<ArgAttrElementT>();
                }
            }
            return ResAttrElementT::get(
                resultType, calculate(lhs.getValue(), rhs.getValue()));
        } else if constexpr (std::is_same<ResAttrElementT,
                                          mlir::BoolAttr>::value) {
            ...
            return ResAttrElementT::get(
                lhs.getContext(), calculate(lhs.getValue(), rhs.getValue()));
        } else if constexpr (std::is_same<ResAttrElementT,
            ...
            return ResAttrElementT::get(
                calculate(lhs.getValue(), rhs.getValue()), resultType);
        }
    }
    return {};
}
...
mlir::Attribute constFoldUnaryOp(mlir::Location loc, mlir::Type resultType,
                                 llvm::ArrayRef<mlir::Attribute> operands,
                                 const CalculationT &calculate) {
    if (operands.size() != 1)
        throw ErrorHandler::compilerError(
            loc, "CanonicalizerPass (constFoldUnaryOp)",
            "unary op takes one operand but " +
                std::to_string(operands.size()) + " were given");
    ...
    if (auto intAttr = attr.dyn_cast<mlir::IntegerAttr>()) {
        ...
        if (targetType.isF64()) {
            if (intAttr.getType().isSignedInteger()) {
                return mlir::FloatAttr::getChecked(
                    loc, targetType,
                    llvm::APIntOps::RoundSignedAPIntToDouble(apInt));
            }
            if (intAttr.getType().isUnsignedInteger() ||
                intAttr.getType().isIndex()) {
                return mlir::FloatAttr::getChecked(
                    loc, targetType, llvm::APIntOps::RoundAPIntToDouble(apInt));
            }
        }
        ...
        if (targetType.isF32()) {
            ...
            if (intAttr.getType().isUnsignedInteger()) {
                return mlir::FloatAttr::get(
                    targetType, llvm::APIntOps::RoundAPIntToFloat(apInt));
            }
        }
...
mlir::OpFoldResult mlir::daphne::EwAddOp::fold(FoldAdaptor adaptor) {
    ...
    auto intOp = [](const llvm::APInt &a, const llvm::APInt &b) {
        return a + b;
    };
    if (auto res = constFoldBinaryOp<FloatAttr>(getLoc(), getType(), operands,
                                                floatOp))
        return res;
    if (auto res = constFoldBinaryOp<IntegerAttr>(getLoc(), getType(), operands,
                                                  intOp))
        return res;
    return {};
}
...
mlir::OpFoldResult mlir::daphne::EwDivOp::fold(FoldAdaptor adaptor) {
    ...
    auto sintOp = [&](const llvm::APInt &a, const llvm::APInt &b) {
        if (b == 0) {
            throw ErrorHandler::compilerError(
                this->getLoc(),
                "CanonicalizerPass (mlir::daphne::EwDivOp::fold)",
                "Can't divide by 0");
        }
        return a.sdiv(b);
    };
    ...
    if (getType().isSignedInteger()) {
        if (auto res = constFoldBinaryOp<IntegerAttr>(getLoc(), getType(),
                                                      operands, sintOp))
            return res;
...
mlir::OpFoldResult mlir::daphne::EwMinusOp::fold(FoldAdaptor adaptor) {
    ...
    if (auto res =
            constFoldUnaryOp<IntegerAttr>(getLoc(), getType(), operands, intOp))
        return res;
    if (auto res =
            constFoldUnaryOp<FloatAttr>(getLoc(), getType(), operands, floatOp))
        return res;
...
mlir::OpFoldResult mlir::daphne::EwModOp::fold(FoldAdaptor adaptor) {
    ...
    auto sintOp = [&](const llvm::APInt &a, const llvm::APInt &b) {
        if (b == 0) {
            throw ErrorHandler::compilerError(
                this->getLoc(),
                "CanonicalizerPass (mlir::daphne::EwModOp::fold)",
                "Can't compute mod 0");
        }
        return a.srem(b);
    };
...
mlir::OpFoldResult mlir::daphne::EwAndOp::fold(FoldAdaptor adaptor) {
    ...
    if (auto res =
            constFoldBinaryOp<BoolAttr>(getLoc(), getType(), operands, boolOp))
        return res;
...
mlir::OpFoldResult mlir::daphne::EwConcatOp::fold(FoldAdaptor adaptor) {
    ...
    if (operands.size() != 2)
        throw ErrorHandler::compilerError(
            this->getLoc(),
            "CanonicalizerPass (mlir::daphne::EwConcatOp::fold)",
            "binary op takes two operands but " +
                std::to_string(operands.size()) + " were given");
...
mlir::OpFoldResult mlir::daphne::EwEqOp::fold(FoldAdaptor adaptor) {
    ...
    auto strOp = [](const llvm::StringRef &a, const llvm::StringRef &b) {
        return a == b;
    };

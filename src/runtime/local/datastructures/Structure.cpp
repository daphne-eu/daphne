...
Structure::Structure(size_t numRows, size_t numCols)
...
void Structure::clone_mdo(const Structure *src) {
    ...
    // for data copy)
    for (int i = 0; i < static_cast<int>(ALLOCATION_TYPE::NUM_ALLOC_TYPES);
         i++) {
        auto placements =
            src->mdo->getDataPlacementByType(static_cast<ALLOCATION_TYPE>(i));
        for (auto it = placements->begin(); it != placements->end(); it++) {
            ...
            auto new_data_placement =
                this->mdo->addDataPlacement(src_alloc, src_range);

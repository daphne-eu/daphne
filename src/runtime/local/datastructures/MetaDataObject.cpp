...
DataPlacement *MetaDataObject::addDataPlacement(std::vector<std::unique_ptr<IAllocationDescriptor>> &allocInfos,
                                                Range *r) {
    if (r) {
        ...
        auto type = static_cast<size_t>(allocInfos.front()->getType());
        data_placements[type] = std::make_unique<DataPlacement>(std::move(allocInfos), nullptr);
...
std::pair<uint32_t, std::byte *>
MetaDataObject::getDataInternal(uint32_t alloc_idx, const IAllocationDescriptor *alloc_desc, const Range *range) {
    DataPlacement *dp;
    if (!range) {
        if (!alloc_desc) {
...
const std::byte *MetaDataObject::getData(uint32_t alloc_idx, const IAllocationDescriptor *alloc_desc,

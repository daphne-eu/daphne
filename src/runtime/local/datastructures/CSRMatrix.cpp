...
CSRMatrix<ValueType>::CSRMatrix(size_t maxNumRows, size_t numCols, size_t maxNumNonZeros, bool zero,
                                IAllocationDescriptor *allocInfo)
    : Matrix<ValueType>(maxNumRows, numCols), numRowsAllocated(maxNumRows), isRowAllocatedBefore(false), is_view(false),
    ...
    spdlog::debug(
        "Creating {} x {} sparse matrix of type: {}. Required memory: vals={}, cidxs={}, rptrs={}, total={} Mb",
        numRows, numCols, val_alloc->getTypeName(), static_cast<float>(val_buf_size) / (1048576),
        static_cast<float>(cidx_buf_size) / (1048576), static_cast<float>(rptr_buf_size) / (1048576),
        static_cast<float>(val_buf_size + cidx_buf_size + rptr_buf_size) / (1048576));

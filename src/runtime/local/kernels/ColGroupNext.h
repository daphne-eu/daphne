...
void colGroupNext(DTResGrpIds *&resGrpIds, DTResReprPos *&resReprPos, const DTArgData *argData,
    ...
    ColGroupNext<DTResGrpIds, DTResReprPos, DTArgData, DTArgGrpIds>::apply(resGrpIds, resReprPos, argData, argGrpIds, ctx);
}
    ...
    static void apply(Column<VTPos> *&resGrpIds, Column<VTPos> *&resReprPos, const Column<VTData> *argData,
        ...
        for (size_t r = 0; r < numArgData; r++) {
            ...
            if (!grpId) { // The value was not found.
                // grpId = grpIds.size() - 1; // -1 because the value has just been inserted.
                grpId = grpIds.size();
                *valuesResReprPos = r;
                valuesResReprPos++;
            }
        ...
        // std::cerr << "colGrpNext: resGrpIds: " << std::endl;
        // resGrpIds->print(std::cerr);
        // std::cerr << "colGrpNext: resReprPos: " << std::endl;
        // resReprPos->print(std::cerr);
    }
};
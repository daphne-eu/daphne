...
static uint32_t getPQ(uint32_t img_extent, uint32_t filter_extent,
                      uint32_t pad_extent, uint32_t stride_extent) {
    uint32_t padded_image_extent = img_extent + 2 * pad_extent;
    return (padded_image_extent - filter_extent) / stride_extent + 1;
}

template <typename VT>
static inline void Padding(VT *padded_input, const VT *input, size_t pad_h,
                           size_t pad_w, size_t img_w, size_t img_h,
                           uint32_t off) {
...
template <typename VT>
static inline void CleanPaddingAndSave(VT *res, const VT *dPooling_padded,
                                       size_t pad_h, size_t pad_w, size_t img_w,

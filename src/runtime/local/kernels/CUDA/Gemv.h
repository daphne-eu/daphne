...
template <typename T>
void launch_cublas_gemv(const CUDAContext &ctx, size_t m, size_t n,
                        const T *alpha, const T *beta, const T *A, const T *x,
                        T *y, cublasOperation_t opA);
...
struct Gemv<DenseMatrix<T>, DenseMatrix<T>, DenseMatrix<T>> {
    static void apply(DenseMatrix<T> *&res, const DenseMatrix<T> *mat,

...
// ToDo: better use of template code for dense/sparse
template<typename VT>
uint32_t writeInputToStagingBuffer(std::byte* bbuf, const CSRMatrix<VT>* input, const uint32_t start_pos,
                                   AllocationDescriptorCUDA* alloc_desc) {
    uint32_t pos = start_pos;
    size_t tmp;
    auto ptr64 = reinterpret_cast<uint64_t*>(&(bbuf[pos]));
...
//    ptr++;
    ...
    // row_ptr
    ptr64 = reinterpret_cast<size_t*>(&(bbuf[pos]));
    auto rtmp = input->getRowOffsets(alloc_desc);
    *ptr64 = reinterpret_cast<size_t>(const_cast<size_t*>(rtmp));
    pos += sizeof(size_t*);

    // col_idxs
    ptr64 = reinterpret_cast<size_t*>(&(bbuf[pos]));
    auto ctmp = input->getColIdxs(alloc_desc);
    *ptr64 = reinterpret_cast<size_t>(const_cast<size_t*>(ctmp));
    pos += sizeof(size_t*);
...
//        dctx->logger->info("daphne context CodeGenRW");
    ...
    auto a = args[0];
    auto b = reinterpret_cast<const DenseMatrix<VTarg>*>(args[1]);
...
//    std::cout << "executing op=" << operator_name << " id=" << opID << std::endl;
    ...
    ctx->logger->debug("codeGenRW sparse: wrote {} bytes in staging buffer", pos);

//    CUDAContext::debugPrintCUDABuffer(*ctx, "arg row ptrs", a->getRowOffsets(&alloc_desc), (a->getNumRows() + 1));
...
void CUDA_codegenCW__float__DenseMatrix_float_variadic__size_t(float *res, const DenseMatrix<float> **arg,
                                                                 size_t num_operands, DCTX(ctx)) {
//        auto nc = ctx->cuda_contexts.size();
//        std::cout << "codegenCW: num operands: " << num_operands << ", num cuda contexts: " << nc << std::endl;
    *res = CodeGenCW<double>(arg, ctx);
...
void CUDA_codegenRW__DenseMatrix_double__DenseMatrix_float_variadic__size_t(DenseMatrix<double> **res,
                                                                             const DenseMatrix<float> **arg, size_t num_operands, DCTX(ctx)) {
//        auto nc = ctx->cuda_contexts.size();
//        std::cout << "codegenRW: num operands: " << num_operands << ", num cuda contexts: " << nc << std::endl;
    CodeGenRW(*res, arg, ctx);
}

void CUDA_codegenRW__DenseMatrix_double__CSRMatrix_double_variadic__size_t(DenseMatrix<double> **res,
                                                                             const CSRMatrix<double> **arg, size_t num_operands, DCTX(ctx)) {
//        auto nc = ctx->cuda_contexts.size();
//        std::cout << "codegenRW sparse: num operands: " << num_operands << ", num cuda contexts: " << nc << std::endl;
    CodeGenRW(*res, arg, ctx);
}

void CUDA_codegenRW__DenseMatrix_float__CSRMatrix_float_variadic__size_t(DenseMatrix<float> **res,
                                                                           const CSRMatrix<float> **arg, size_t num_operands, DCTX(ctx)) {
//        auto nc = ctx->cuda_contexts.size();
//        std::cout << "codegenRW sparse: num operands: " << num_operands << ", num cuda contexts: " << nc << std::endl;
    CodeGenRW(*res, arg, ctx);
}

void CUDA_codegenRW__DenseMatrix_double__CSRMatrix_float_variadic__size_t(DenseMatrix<double> **res,
        const CSRMatrix<float> **arg, size_t num_operands, DCTX(ctx)) {
//        auto nc = ctx->cuda_contexts.size();
//        std::cout << "codegenRW sparse: num operands: " << num_operands << ", num cuda contexts: " << nc << std::endl;
    CodeGenRW(*res, arg, ctx);

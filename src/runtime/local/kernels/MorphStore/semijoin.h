...
public:
    ...
    static void apply(Frame * & res, const Frame * inLeft, const Frame * inRight, const char ** inOnLeft, size_t numLhsOn, const char ** inOnRight, size_t numRhsOn) {
        ...
        for (size_t i = 0; i < numLhsOn; ++ i) {
            ...
            if (selectPos) {
                auto tempPos = morphstore::intersect_sorted<ve, morphstore::uncompr_f, morphstore::uncompr_f, morphstore::uncompr_f >(selectPos, currentPosCol);
                delete selectPos;
                selectPos = tempPos;
            } else {
                selectPos = currentPosCol;
            }

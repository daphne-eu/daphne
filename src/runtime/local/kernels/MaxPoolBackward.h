...
void maxPoolBackward(DTRes *&res, const DTArg *input, const DTArg *dOut,
    ...
    MaxPoolBackward<DTRes, DTArg>::apply(
    ...
    static void apply(DenseMatrix<VTRes> *&res, const DenseMatrix<VTArg> *input,
        ...
        for (uint32_t i = start; i < stop; i++)
            for (uint32_t c = 0; c < C; c++) {
                ...
                Padding(input_padded->getValues(), input->getValues(), pad_h,
                        pad_w, img_w, img_h, off_input);
                for (uint32_t p = 0; p < P; p++)
                    for (uint32_t q = 0; q < Q; q++) {
                        auto max_index =
                            p * stride_h * padded_img_w + q * stride_w;
                        for (uint32_t h = 0; h < pool_h; h++)
                            for (uint32_t w = 0; w < pool_w; w++) {
                                auto off_padded =
                                    (p * stride_h + h) * padded_img_w +
                                    q * stride_w + w;
                                if (input_padded->getValues()[off_padded] >
                                    input_padded->getValues()[max_index])
                ...
                CleanPaddingAndSave(res->getValues(), res_padded->getValues(),
                                    pad_h, pad_w, img_w, img_h, off_input);

...
// ----------------------------------------------------------------------------
// ContiguousTensor <- ContiguousTensor, ContiguousTensor, scalar
// ----------------------------------------------------------------------------

template<typename VTVal, typename VTCond>
struct CondMatMatSca<ContiguousTensor<VTVal>, ContiguousTensor<VTCond>, ContiguousTensor<VTVal>, VTVal> {
    static void apply(
        ...
        for (size_t i=0; i < cond->total_element_count; i++) {
            res->data[i] = static_cast<bool>(cond->data[i]) ? thenVal->data[i] : elseVal;
        }
    }
};

// ----------------------------------------------------------------------------
// ChunkedTensor <- ChunkedTensor, ChunkedTensor, scalar
// ----------------------------------------------------------------------------

template<typename VTVal, typename VTCond>
struct CondMatMatSca<ChunkedTensor<VTVal>, ChunkedTensor<VTCond>, ChunkedTensor<VTVal>, VTVal> {
    static void apply(
        ...
        for (size_t i=0; i < cond->total_chunk_count; i++) {
            ...
            if (!cond->isPartialChunk(current_chunk_ids)) {
                for (size_t j=0; j < cond->chunk_element_count; j++) {
                    current_chunk_ptr_res[j] = static_cast<bool>(current_chunk_ptr_cond[j]) ? current_chunk_ptr_thenVal[j] : elseVal; 
                }
            } else {
                ...
                for (size_t i=0; i < valid_element_count; i++) {
                    ...
                    current_chunk_ptr_res[lin_id] = static_cast<bool>(current_chunk_ptr_cond[lin_id]) ? current_chunk_ptr_thenVal[lin_id] : elseVal;
                }
            }

...
template <class DTRes, class DTCond, class DTThen, class VTElse>
void condMatMatSca(DTRes *&res, const DTCond *cond, const DTThen *thenVal,
                   VTElse elseVal, DCTX(ctx)) {
    CondMatMatSca<DTRes, DTCond, DTThen, VTElse>::apply(res, cond, thenVal,
                                                        elseVal, ctx);
}
...
struct CondMatMatSca<DenseMatrix<VTVal>, DenseMatrix<VTCond>,
                     DenseMatrix<VTVal>, VTVal> {
    static void apply(DenseMatrix<VTVal> *&res, const DenseMatrix<VTCond> *cond,
                      const DenseMatrix<VTVal> *thenVal, VTVal elseVal,
                      DCTX(ctx)) {
        ...
        for (size_t r = 0; r < numRows; r++) {
            for (size_t c = 0; c < numCols; c++)
                valuesRes[c] =
                    static_cast<bool>(valuesCond[c]) ? valuesThen[c] : elseVal;
...
struct CondMatMatSca<Matrix<VTVal>, Matrix<VTCond>, Matrix<VTVal>, VTVal> {
    static void apply(Matrix<VTVal> *&res, const Matrix<VTCond> *cond,
                      const Matrix<VTVal> *thenVal, VTVal elseVal, DCTX(ctx)) {
        const size_t numRows = cond->getNumRows();
        const size_t numCols = cond->getNumCols();

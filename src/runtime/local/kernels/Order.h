...
template <class DTRes, class DTArg> struct Order {
    static void
    apply(DTRes *&res, const DTArg *arg, size_t *colIdxs, size_t numColIdxs,
...
void order(DTRes *&res, const DTArg *arg, size_t *colIdxs, size_t numColIdxs,
           bool *ascending, size_t numAscending, bool returnIdx, DCTX(ctx),
           std::vector<std::pair<size_t, size_t>> *groupsRes = nullptr) {
    Order<DTRes, DTArg>::apply(res, arg, colIdxs, numColIdxs, ascending,
                               numAscending, returnIdx, ctx, groupsRes);
}
...
void columnIDSort(DenseMatrix<VTIdx> *&idx, const DenseMatrix<VT> *arg,
                  size_t colIdx, std::vector<std::pair<VTIdx, VTIdx>> &groups,
                  bool ascending, DCTX(ctx)) {
    VTIdx *indices = idx->getValues();
    const VT *values = arg->getValues();
    const size_t rowSkip = arg->getRowSkip();
    auto compare =
        ascending ? std::function{[&values, rowSkip, colIdx](VTIdx i, VTIdx j) {
            return values[i * rowSkip + colIdx] < values[j * rowSkip + colIdx];
        }}
                  : std::function{[&values, rowSkip, colIdx](VTIdx i, VTIdx j) {
                        return values[i * rowSkip + colIdx] >
                               values[j * rowSkip + colIdx];
                    }};
    for (const auto &group : groups)
        std::stable_sort(indices + group.first, indices + group.second,
                         compare);
}

// function overload for generic Matrix type
template <typename VTIdx, typename VT>
void columnIDSort(DenseMatrix<VTIdx> *&idx, const Matrix<VT> *arg,
                  size_t colIdx, std::vector<std::pair<VTIdx, VTIdx>> &groups,
                  bool ascending, DCTX(ctx)) {
    VTIdx *indices = idx->getValues();
...
// function overload for generic Matrix type
template <typename VT>
size_t nextWithRowskip(const size_t firstIdx, const size_t lastIdx,
                       const size_t colIdx, Matrix<VT> *arg) {
...
template <typename VTIdx, typename VT>
void columnGroupScan(std::vector<std::pair<VTIdx, VTIdx>> &groups,
                     DenseMatrix<VT> *col, const size_t colIdx, DCTX(ctx)) {
    ...
    for (size_t g = 0; g < numOldGroups; g++) {
        ...
        while (first != last) {
            ...
            if ((size_t)(next - first) > rowSkip)
                groups.push_back(
                    std::make_pair((first - values - colIdx) / rowSkip,
                                   (next - values - colIdx) / rowSkip));
            first = next;
        }
    }
...
template <typename VTIdx, typename VT>
void columnGroupScan(std::vector<std::pair<VTIdx, VTIdx>> &groups,
                     Matrix<VT> *col, const size_t colIdx, DCTX(ctx)) {
    const size_t numOldGroups = groups.size();
    for (size_t g = 0; g < numOldGroups; ++g) {
        ...
        while (currentRowIdx != lastRowIdx) {
            size_t nextIdx =
                nextWithRowskip<VT>(currentRowIdx, lastRowIdx, colIdx, col);
...
void multiColumnIDSort(DenseMatrix<VTIdx> *&idx, const DTCol *col,
                       size_t colIdx,
                       std::vector<std::pair<VTIdx, VTIdx>> &groups,
                       bool ascending, DCTX(ctx)) {
...
template <typename VTCol> struct ColumnIDSort {
    static void apply(const Frame *arg, DenseMatrix<size_t> *&idx,
                      std::vector<std::pair<size_t, size_t>> &groups,
                      bool ascending, size_t colIdx, DCTX(ctx)) {
        columnIDSort(idx, arg->getColumn<VTCol>(colIdx), 0, groups, ascending,
                     ctx);
    }
};
...
template <typename VTCol> struct MultiColumnIDSort {
    static void apply(const Frame *arg, DenseMatrix<size_t> *&idx,
                      std::vector<std::pair<size_t, size_t>> &groups,
                      bool ascending, size_t colIdx, DCTX(ctx)) {
        multiColumnIDSort(idx, arg->getColumn<VTCol>(colIdx), 0, groups,
                          ascending, ctx);
    }
};

struct OrderFrame {
    static void apply(DenseMatrix<size_t> *&idx, const Frame *arg,
                      size_t *colIdxs, size_t numColIdxs, bool *ascending,
                      size_t numAscending,
                      std::vector<std::pair<size_t, size_t>> *groupsRes,
                      DCTX(ctx)) {
        ...
        if (numColIdxs > 1) {
            for (size_t i = 0; i < numColIdxs - 1; i++) {
                DeduceValueTypeAndExecute<MultiColumnIDSort>::apply(
                    arg->getSchema()[colIdxs[i]], arg, idx, groups,
        ...
        if (groupsRes == nullptr) {
            DeduceValueTypeAndExecute<ColumnIDSort>::apply(
                arg->getSchema()[colIdx], arg, idx, groups,
...
template <> struct Order<Frame, Frame> {
    static void
    apply(Frame *&res, const Frame *arg, size_t *colIdxs, size_t numColIdxs,
          bool *ascending, size_t numAscending, bool returnIdx, DCTX(ctx),
          std::vector<std::pair<size_t, size_t>> *groupsRes = nullptr) {
        if (arg == nullptr || colIdxs == nullptr || numColIdxs == 0 ||
            ascending == nullptr || returnIdx) {
            throw std::runtime_error(
                "order-kernel called with invalid arguments");
        }
        DenseMatrix<size_t> *idx = nullptr;
        OrderFrame::apply(idx, arg, colIdxs, numColIdxs, ascending,
                          numAscending, groupsRes, ctx);
...
template <typename VTRes> struct Order<DenseMatrix<VTRes>, Frame> {
    static void
    apply(DenseMatrix<VTRes> *&res, const Frame *arg, size_t *colIdxs,
          size_t numColIdxs, bool *ascending, size_t numAscending,
          bool returnIdx, DCTX(ctx),
          std::vector<std::pair<size_t, size_t>> *groupsRes = nullptr) {
        if (arg == nullptr || colIdxs == nullptr || numColIdxs == 0 ||
            ascending == nullptr || !returnIdx) {
            throw std::runtime_error(
...
struct Order<DenseMatrix<VTRes>, DenseMatrix<VTArg>> {
    static void
    apply(DenseMatrix<VTRes> *&res, const DenseMatrix<VTArg> *arg,
          size_t *colIdxs, size_t numColIdxs, bool *ascending,
        ...
        auto idx =
            DataObjectFactory::create<DenseMatrix<size_t>>(numRows, 1, false);
        auto indices = idx->getValues();
        std::iota(indices, indices + numRows, 0);
        std::vector<std::pair<size_t, size_t>> groups;
        groups.push_back(std::make_pair(0, numRows));
        ...
        if (numColIdxs > 1) {
            for (size_t i = 0; i < numColIdxs - 1; i++) {
                multiColumnIDSort(idx, arg, colIdxs[i], groups, ascending[i],
                                  ctx);
            }
        }

        if (groupsRes == nullptr) {
            columnIDSort(idx, arg, colIdxs[numColIdxs - 1], groups,
                         ascending[numColIdxs - 1], ctx);
        } else {
            multiColumnIDSort(idx, arg, colIdxs[numColIdxs - 1], groups,
                              ascending[numColIdxs - 1], ctx);
            groupsRes->insert(groupsRes->end(), groups.begin(), groups.end());
        }
...
struct Order<Matrix<VTRes>, Matrix<VTArg>> {
    static void
    apply(Matrix<VTRes> *&res, const Matrix<VTArg> *arg, size_t *colIdxs,
          size_t numColIdxs, bool *ascending, size_t numAscending,
          bool returnIdx, DCTX(ctx),
          std::vector<std::pair<size_t, size_t>> *groupsRes = nullptr) {
        size_t numRows = arg->getNumRows();
        ...
        if (returnIdx) {
            if (res == nullptr)
                res = DataObjectFactory::create<DenseMatrix<VTRes>>(numRows, 1,
                                                                    false);

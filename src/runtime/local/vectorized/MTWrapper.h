...
using mlir::daphne::VectorCombine;
    ...
    protected:
        ...
        void initCPPWorkers(std::vector<TaskQueue *> &qvector, uint32_t batchSize, const bool verbose = false,
            ...
            for (auto &w: cpp_workers) {
                w = std::make_unique<WorkerCPU>(qvector, topologyPhysicalIds, topologyUniqueThreads, verbose, 0,
                                                batchSize,
    ...
    public:
        using PipelineFunc = void(DT ***, Structure **, DCTX(ctx));
...
#ifndef NDEBUG
            std::cerr << "spawning " << this->_numCPPThreads << " CPU and " << this->_numCUDAThreads
                      << " CUDA worker threads"
                      << std::endl;
#endif
        }
...
//        using PipelineFunc = typename MTWrapperBase<DenseMatrix<VT>>::PipelineFunc;
    ...
    public:
        using PipelineFunc = typename MTWrapperBase<DenseMatrix<VT>>::PipelineFunc;
        using MTWrapperBase<DenseMatrix<VT>>::getInputProperties;
...
//    using PipelineFunc = void(DenseMatrix<VT> ***, Structure **, DCTX(ctx));
        ...
        void executeCpuQueues(std::vector<std::function<PipelineFunc>> funcs, DenseMatrix<VT> ***res,
...
//    }
        ...
        void executeCpuQueues(std::vector<std::function<PipelineFunc>> funcs, CSRMatrix<VT> ***res, bool *isScalar,
                Structure **inputs, size_t numInputs, size_t numOutputs, const int64_t *outRows, const int64_t *outCols,

...
template <typename DT> class MTWrapperBase {
  protected:
    ...
    std::pair<size_t, size_t> getInputProperties(Structure **inputs,
                                                 size_t numInputs,
                                                 VectorSplit *splits) {
        auto len = 0ul;
        ...
        for (auto i = 0u; i < numInputs; ++i) {
            if (splits[i] == mlir::daphne::VectorSplit::ROWS) {
                ...
                mem_required +=
                    inputs[i]->getNumItems() * sizeof(typename DT::VT);
            }
        }
    ...
    void hwloc_recurse_topology(hwloc_topology_t topo, hwloc_obj_t obj,
        ...
        if (obj->type != HWLOC_OBJ_CORE) {
            for (unsigned int i = 0; i < obj->arity; i++) {
                hwloc_recurse_topology(topo, obj->children[i],
                                       parent_package_id, physicalIds,
                                       uniqueThreads, responsibleThreads);
            }
        } else {
    ...
    void get_topology(std::vector<int> &physicalIds,
                      std::vector<int> &uniqueThreads,
                      std::vector<int> &responsibleThreads) {
        ...
        while (package != NULL) {
            ...
            hwloc_recurse_topology(topology, package, package_id, physicalIds,
                                   uniqueThreads, responsibleThreads);
            package = hwloc_get_next_obj_by_type(topology, HWLOC_OBJ_PACKAGE,
                                                 package);
        }
    ...
    void initCPPWorkers(std::vector<TaskQueue *> &qvector, uint32_t batchSize,
                        const bool verbose = false, int numQueues = 0,
                        int queueMode = 0, bool pinWorkers = false) {
        ...
        for (auto &w : cpp_workers) {
            w = std::make_unique<WorkerCPU>(
                qvector, topologyPhysicalIds, topologyUniqueThreads, _ctx,
                verbose, 0, batchSize, i, numQueues, queueMode,
                this->_stealLogic, pinWorkers);
            i++;
        }
    }
#ifdef USE_CUDA
    void initCUDAWorkers(TaskQueue *q, uint32_t batchSize,
                         bool verbose = false) {
    ...
    void cudaPrefetchInputs(Structure **inputs, uint32_t numInputs,
                            size_t mem_required,
        ...
        auto buffer_usage = static_cast<float>(mem_required) /
                            static_cast<float>(ctx->getMemBudget());
        ctx->logger->debug("Vect pipe total in/out buffer usage: {}",
                           buffer_usage);
        if (buffer_usage < 1.0) {
            for (auto i = 0u; i < numInputs; ++i) {
                if (splits[i] == mlir::daphne::VectorSplit::ROWS) {
                    [[maybe_unused]] auto unused =
                        static_cast<const DT *>(inputs[i])->getValues(
                            &alloc_desc);
                }
            }
        }
...
#endif
    size_t allocateOutput(DT ***&res, size_t numOutputs, const int64_t *outRows,
    ...
    virtual void combineOutputs(DT ***&res, DT ***&res_cuda, size_t numOutputs,
                                mlir::daphne::VectorCombine *combines,
                                DCTX(ctx)) = 0;
  ...
  public:
    explicit MTWrapperBase(uint32_t numFunctions, DCTX(ctx)) : _ctx(ctx) {
        ...
        if (_ctx->config.debugMultiThreading) {
            ...
            std::cout << std::endl
                      << "topologyResponsibleThreads:" << std::endl;
            for (const auto &topologyEntry : topologyResponsibleThreads) {
                std::cout << topologyEntry << ',';
            }
            std::cout << std::endl
                      << "_totalNumaDomains=" << _totalNumaDomains << std::endl;
            std::cout << "_numQueues=" << _numQueues << std::endl;
        }

        _ctx->logger->debug("spawning {} CPU and {} CUDA worker threads",
                            this->_numCPPThreads, this->_numCUDAThreads);
    }
...
class MTWrapper<DenseMatrix<VT>> : public MTWrapperBase<DenseMatrix<VT>> {
  public:
    ...
    explicit MTWrapper(uint32_t numFunctions, DCTX(ctx))
        : MTWrapperBase<DenseMatrix<VT>>(numFunctions, ctx) {}

    [[maybe_unused]] void
    executeSingleQueue(std::vector<std::function<PipelineFunc>> funcs,
                       DenseMatrix<VT> ***res, const bool *isScalar,
                       Structure **inputs, size_t numInputs, size_t numOutputs,
                       int64_t *outRows, int64_t *outCols, VectorSplit *splits,
                       VectorCombine *combines, DCTX(ctx), bool verbose);

    [[maybe_unused]] void
    executeCpuQueues(std::vector<std::function<PipelineFunc>> funcs,
                     DenseMatrix<VT> ***res, const bool *isScalar,
                     Structure **inputs, size_t numInputs, size_t numOutputs,
                     int64_t *outRows, int64_t *outCols, VectorSplit *splits,
                     VectorCombine *combines, DCTX(ctx), bool verbose);

    [[maybe_unused]] void executeQueuePerDeviceType(
        std::vector<std::function<PipelineFunc>> funcs, DenseMatrix<VT> ***res,
        const bool *isScalar, Structure **inputs, size_t numInputs,
        size_t numOutputs, int64_t *outRows, int64_t *outCols,
        VectorSplit *splits, VectorCombine *combines, DCTX(ctx), bool verbose);

    void combineOutputs(DenseMatrix<VT> ***&res, DenseMatrix<VT> ***&res_cuda,
                        size_t numOutputs,
                        mlir::daphne::VectorCombine *combines,
                        DCTX(ctx)) override;
};
...
class MTWrapper<CSRMatrix<VT>> : public MTWrapperBase<CSRMatrix<VT>> {
  public:
    ...
    [[maybe_unused]] void executeSingleQueue(
        std::vector<std::function<PipelineFunc>> funcs, CSRMatrix<VT> ***res,
        const bool *isScalar, Structure **inputs, size_t numInputs,
        size_t numOutputs, const int64_t *outRows, const int64_t *outCols,
        VectorSplit *splits, VectorCombine *combines, DCTX(ctx), bool verbose) {
        throw std::runtime_error(
            "sparse single queue vect exec not implemented");
    }

    [[maybe_unused]] void executeCpuQueues(
        std::vector<std::function<PipelineFunc>> funcs, CSRMatrix<VT> ***res,
        const bool *isScalar, Structure **inputs, size_t numInputs,
        size_t numOutputs, const int64_t *outRows, const int64_t *outCols,
        VectorSplit *splits, VectorCombine *combines, DCTX(ctx), bool verbose);
    ...
    [[maybe_unused]] void executeQueuePerDeviceType(
        ...
        VectorSplit *splits, VectorCombine *combines, DCTX(ctx), bool verbose) {
        throw std::runtime_error(
            "sparse queuePerDeviceType vect exec not implemented");
    }

    void combineOutputs(CSRMatrix<VT> ***&res, CSRMatrix<VT> ***&res_cuda,
                        [[maybe_unused]] size_t numOutputs,

...
namespace CUDA {
    ...
    [[maybe_unused]] void MTWrapper<DenseMatrix<VT>>::executeQueuePerDeviceType(
        ...
        CUDA::MTWrapper<DenseMatrix<VT>>::initCUDAWorkers(q_cuda.get(), batchSize8M * 4, verbose);
...
// End Multiple Queues
            ...
            while (lp.hasNextChunk()) {
                ...
                qvector[target]->enqueueTask(
                        new CompiledPipelineTask<DenseMatrix<VT>>(CompiledPipelineTaskData<DenseMatrix<VT>>{
                                funcs, isScalar, inputs, numInputs, numOutputs, outRows, outCols, splits, combines,
                                startChunk, endChunk,
...
//                auto data_src = src_base_ptr + res_cuda->getRowSkip() * j;
    ...
    template
    class MTWrapper<DenseMatrix<double>>;

    template
    class MTWrapper<DenseMatrix<float>>;
}

extern "C" {
    IVectorizedExecutor* cuda_create_vectorized_executor() {
        return new CUDAVectorizedExecutor();
    }

    void cuda_destroy_vectorized_executor(IVectorizedExecutor* p) {
        delete p;

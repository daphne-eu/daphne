...
template <ALLOCATION_TYPE AT, class DT> struct DistributedCollect {
    static void apply(DT *&mat, const VectorCombine &combine,
                      DCTX(dctx)) = delete;
};
    ...
    static void apply(DT *&mat, const VectorCombine &combine, DCTX(dctx)) {
        if (mat == nullptr)
            throw std::runtime_error(
                "DistributedCollect gRPC: result matrix must be already "
        ...
        for (size_t rank = 0; rank < worldSize; rank++) {
            ...
            auto distributedData =
                dynamic_cast<AllocationDescriptorMPI &>(*(dp->allocation))
                    .getDistributedData();
            WorkerImpl::StoredInfo info = {distributedData.identifier,
        ...
        for (size_t i = 1; i < worldSize; i++) {
            ...
            MPIHelper::getMessage(&rank, TypesOfMessages::OUTPUT,
                                  MPI_UNSIGNED_CHAR, buffer, &len);
            ...
            auto slicedMat =
                dynamic_cast<DenseMatrix<double> *>(DF_deserialize(buffer));
            if (combine == VectorCombine::ADD) {
                ewBinaryMat(BinaryOpCode::ADD, denseMat, slicedMat, denseMat,
                            nullptr);
            } else {
                auto resValues = denseMat->getValues() +
                                 (dp->range->r_start * denseMat->getRowSkip());
                auto slicedMatValues = slicedMat->getValues();
                for (size_t r = 0; r < dp->range->r_len; r++) {
                    memcpy(resValues + dp->range->c_start, slicedMatValues,
                           dp->range->c_len * sizeof(double));
            ...
            dynamic_cast<AllocationDescriptorMPI &>(*(dp->allocation))
                .updateDistributedData(distributedData);
            ...
            // work items
            if (collectedDataItems ==
                denseMat->getNumRows() * denseMat->getNumCols())
                break;
        }
    };
};
...
struct DistributedCollect<ALLOCATION_TYPE::DIST_GRPC_ASYNC, DT> {
    static void apply(DT *&mat, const VectorCombine &combine, DCTX(dctx)) {
        ...
        DistributedGRPCCaller<StoredInfo, distributed::StoredData,
                              distributed::Data>
            caller(dctx);

        auto dpVector = mat->getMetaDataObject()->getDataPlacementByType(
            ALLOCATION_TYPE::DIST_GRPC);
        ...
        while (!caller.isQueueEmpty()) {
            ...
            // Zero copy buffer
            std::vector<char> buf(
                static_cast<const char *>(matProto.bytes().data()),
                static_cast<const char *>(matProto.bytes().data()) +
                    matProto.bytes().size());
            auto slicedMat =
                dynamic_cast<DenseMatrix<double> *>(DF_deserialize(buf));
            if (combine == VectorCombine::ADD) {
                ewBinaryMat(BinaryOpCode::ADD, denseMat, slicedMat, denseMat,
                            nullptr);
            } else {
...
struct DistributedCollect<ALLOCATION_TYPE::DIST_GRPC_SYNC, DT> {
    static void apply(DT *&mat, const VectorCombine &combine, DCTX(dctx)) {
        ...
        for (auto &dp : *dpVector) {
            ...
            std::thread t([address, dp = dp.get(), protoData, distributedData,
                           &combine, &lock, &mat, &ctx]() mutable {
                ...
                if (combine == VectorCombine::ADD) {
                    ...
                    ewBinaryMat(BinaryOpCode::ADD, denseMat, slicedMat,
                                denseMat, nullptr);
                } else {
                    auto resValues =
                        denseMat->getValues() +

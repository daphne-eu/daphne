...
class VectorizedPipelineOpLowering : public OpConversionPattern<daphne::VectorizedPipelineOp> {
  ...
  public:
    ...
    LogicalResult matchAndRewrite(daphne::VectorizedPipelineOp op, OpAdaptor adaptor,
        ...
        auto ptrI1Ty = LLVM::LLVMPointerType::get(op.getContext());
        auto ptrPtrI1Ty = LLVM::LLVMPointerType::get(op.getContext());
        auto pppI1Ty = LLVM::LLVMPointerType::get(op.getContext());
        ...
        LLVM::LLVMFuncOp fOp;
        {
            ...
            for (auto i = 0u; i < numDataOperands; ++i) {
                ...
                SmallVector<LLVM::GEPArg, 1> idx = {static_cast<int32_t>(i)};
                auto addr = rewriter.create<LLVM::GEPOp>(loc, ptrPtrI1Ty, ptrI1Ty, inputsArg, idx);
            ...
            for (auto i = 0u; i < oldReturn->getNumOperands(); ++i) {
                ...
                // written better
                auto addr1 = rewriter.create<LLVM::GEPOp>(op->getLoc(), pppI1Ty, ptrPtrI1Ty, returnRef, idx);

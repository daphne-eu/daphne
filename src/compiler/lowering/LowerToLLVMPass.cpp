        ...
        if (auto strAttr = op.getValue().dyn_cast<StringAttr>()) {
            ...
            auto allocaOp = rewriter.replaceOpWithNewOp<LLVM::AllocaOp>(
                ...
                rewriter.create<arith::ConstantOp>(
                    loc, rewriter.getI64IntegerAttr(numChars)),
                1);
            ...
            for (size_t i = 0; i < numChars; i++) {
                ...
                rewriter.create<LLVM::StoreOp>(
                    ...
                    rewriter.create<LLVM::GEPOp>(op->getLoc(), i8PtrType,
                                                 allocaOp, indices));
            }
#else
            ...
            if (auto iTy = op.getType().dyn_cast<IntegerType>()) {
                ...
                rewriter.replaceOpWithNewOp<arith::ConstantOp>(
                    op.getOperation(), ty,
                    IntegerAttr::get(
                        ty, op.getValue().cast<IntegerAttr>().getValue()));
            } else {
                rewriter.replaceOpWithNewOp<arith::ConstantOp>(
                    op.getOperation(), op.getValue());
            }
#endif
        }
...
class CallKernelOpLowering : public OpConversionPattern<daphne::CallKernelOp> {
    ...
    getLLVMInputOutputTypes(Location &loc, MLIRContext *context,
                            TypeConverter *typeConverter, TypeRange resultTypes,
                            TypeRange operandTypes, bool hasVarRes,
                            Type indexType) {
        ...
        if (hasVarRes) { // combine all results into one variadic result
            ...
            Type mt0 =
                t0.dyn_cast<daphne::MatrixType>().withSameElementTypeAndRepr();
            ...
            for (auto type : resultTypes) {
                if (typeConverter->isLegal(type)) {
                    ...
                    emitError(loc)
                        << "Couldn't convert result type `" << type << "`\n";
            }
        ...
        for (auto type : operandTypes) {
            if (typeConverter->isLegal(type)) {
                ...
                emitError(loc)
                    << "Couldn't convert operand type `" << type << "`\n";
        }
  ...
  public:
    ...
    matchAndRewrite(daphne::CallKernelOp op, OpAdaptor adaptor,
        ...
        const bool hasVarRes = op->hasAttr(ATTR_HASVARIADICRESULTS)
                                   ? op->getAttr(ATTR_HASVARIADICRESULTS)
                                         .dyn_cast<BoolAttr>()
                                         .getValue()
                                   : false;
  ...
  private:
    ...
    dereferenceOutputs(Location &loc, PatternRewriter &rewriter,
                       ModuleOp &module, size_t numResults, bool hasVarRes,
            ...
            for (size_t i = 0; i < numResults; i++) {
                ...
                results.push_back(rewriter.create<LLVM::LoadOp>(
                    loc, rewriter.create<LLVM::GEPOp>(
                             loc, kernelOperands[0].getType(),
                             kernelOperands[0], indices)));
            }
        } else // typical case
            for (size_t i = 0; i < numResults; i++) {
                // dereference output
    ...
    std::vector<Value> allocOutputReferences(Location &loc,
                                             PatternRewriter &rewriter,
                                             ValueRange operands,
                                             std::vector<Type> inputOutputTypes,
                                             size_t numRes, bool hasVarRes,
        ...
        if (hasVarRes) { // combine all results into one variadic result
            ...
            auto allocaOp = rewriter.create<LLVM::AllocaOp>(
                ...
                rewriter
                    .create<arith::ConstantOp>(
                        loc, rewriter.getI64IntegerAttr(numRes))
                    .getResult());
            ...
            // required.
            Type elType = inputOutputTypes[0]
            ...
            if (llvm::isa<LLVM::LLVMPointerType>(elType)) {
                for (size_t i = 0; i < numRes; i++) {
                    ...
                    rewriter.create<LLVM::StoreOp>(
                        ...
                        rewriter.create<LLVM::GEPOp>(loc, inputOutputTypes[0],
                                                     allocaOp, indices));
                }
            }
        } else { // typical case
            ...
            ipHere = rewriter.saveInsertionPoint();
            rewriter.restoreInsertionPoint(ipFuncStart);
            Value cst1 = rewriter.create<arith::ConstantOp>(
            ...
            for (size_t i = 0; i < numRes; i++) {
                ...
                Type elType = inputOutputTypes[i]
                                  .dyn_cast<LLVM::LLVMPointerType>()
                                  .getElementType();
                if (llvm::isa<LLVM::LLVMPointerType>(elType)) {
                    rewriter.create<LLVM::StoreOp>(
        ...
        if (hasVarRes)
            ...
            // constant.
            kernelOperands.push_back(rewriter.create<arith::ConstantOp>(
                loc, rewriter.getIndexAttr(numRes)));
 ...
 */
class CreateVariadicPackOpLowering
    : public OpConversionPattern<daphne::CreateVariadicPackOp> {
  public:
    using OpConversionPattern::OpConversionPattern;
    ...
    matchAndRewrite(daphne::CreateVariadicPackOp op, OpAdaptor adaptor,
        ...
        Type contType = op.getRes()
                            .getType()
                            .dyn_cast<daphne::VariadicPackType>()
                            .getContainedType();
 ...
 */
class StoreVariadicPackOpLowering
    : public OpConversionPattern<daphne::StoreVariadicPackOp> {
  public:
    using OpConversionPattern::OpConversionPattern;
    ...
    matchAndRewrite(daphne::StoreVariadicPackOp op, OpAdaptor adaptor,
        ...
        auto elementType =
            pack.getType().cast<LLVM::LLVMPointerType>().getElementType();
        std::vector<Value> indices = {
            rewriter.create<arith::ConstantOp>(loc, op.getPosAttr())};
        auto addr =
            rewriter.create<LLVM::GEPOp>(loc, pack.getType(), pack, indices);
        Type itemType = item.getType();
        if (itemType != elementType) {
            if (llvm::isa<LLVM::LLVMPointerType>(elementType)) {
                if (itemType.isSignedInteger())
                    item = rewriter.create<LLVM::SExtOp>(
                        loc, rewriter.getI64Type(), item);
                else if (itemType.isUnsignedInteger() ||
                         itemType.isSignlessInteger())
                    item = rewriter.create<LLVM::ZExtOp>(
                        loc, rewriter.getI64Type(), item);
                else if (llvm::isa<FloatType>(itemType)) {
                    item = rewriter.create<LLVM::FPExtOp>(
                        loc, rewriter.getF64Type(), item);
                    item = rewriter.create<LLVM::BitcastOp>(
                        loc, rewriter.getI64Type(), item);
                } else {
                    throw ErrorHandler::compilerError(
                        loc, "LowerToLLVMPass",
                ...
                item =
                    rewriter.create<LLVM::IntToPtrOp>(loc, elementType, item);
            } else {
                throw ErrorHandler::compilerError(
                    loc, "LowerToLLVMPass",
        ...
        rewriter.replaceOpWithNewOp<LLVM::StoreOp>(op.getOperation(), item,
                                                   addr);
        return success();
    }
};
...
class MapOpLowering : public OpConversionPattern<daphne::MapOp> {
  public:
    ...
    matchAndRewrite(daphne::MapOp op, OpAdaptor adaptor,
        ...
        // Result Matrix
        callee << "__"
               << CompilerUtils::mlirTypeToCppTypeName(op.getType(), false);

        // Input Matrix
        callee << "__"
               << CompilerUtils::mlirTypeToCppTypeName(op.getArg().getType(),
                                                       false);
        ...
        auto kernel = rewriter.create<daphne::CallKernelOp>(
            loc, callee.str(), kernelOperands, op->getResultTypes());
...
class VectorizedPipelineOpLowering
    : public OpConversionPattern<daphne::VectorizedPipelineOp> {
    const DaphneUserConfig &cfg;

  public:
    explicit VectorizedPipelineOpLowering(TypeConverter &typeConverter,
                                          MLIRContext *context,
                                          const DaphneUserConfig &cfg)
    ...
    matchAndRewrite(daphne::VectorizedPipelineOp op, OpAdaptor adaptor,
        ...
        LLVM::LLVMFuncOp fOp;
        {
            ...
            auto returnRef =
                funcBlock.addArgument(pppI1Ty, rewriter.getUnknownLoc());
            auto inputsArg =
                funcBlock.addArgument(ptrPtrI1Ty, rewriter.getUnknownLoc());
            auto daphneContext =
                funcBlock.addArgument(ptrI1Ty, rewriter.getUnknownLoc());
            ...
            for (auto callKernelOp : funcBlock.getOps<daphne::CallKernelOp>()) {
                callKernelOp.setOperand(callKernelOp.getNumOperands() - 1,
                                        daphneContext);
            }
            ...
            for (auto i = 0u; i < numDataOperands; ++i) {
                auto addr = rewriter.create<LLVM::GEPOp>(
                    ...
                    ArrayRef<Value>({rewriter.create<arith::ConstantOp>(
                        loc, rewriter.getI64IntegerAttr(i))}));
                Value val = rewriter.create<LLVM::LoadOp>(loc, addr);
                auto expTy =
                    typeConverter->convertType(op.getInputs().getType()[i]);
            ...
            for (auto i = 0u; i < oldReturn->getNumOperands(); ++i) {
                ...
                Value retValConverted =
                    typeConverter->materializeTargetConversion(
                        rewriter, oldReturn->getLoc(),
                        typeConverter->convertType(retVal.getType()), {retVal});
        ...
        if (cfg.use_cuda && !op.getCuda().getBlocks().empty()) {
            LLVM::LLVMFuncOp fOp2;
            {
                ...
                fOp2 =
                    rewriter.create<LLVM::LLVMFuncOp>(loc, funcName, funcType);
                ...
                for (auto callKernelOp :
                     funcBlock.getOps<daphne::CallKernelOp>()) {
                    callKernelOp.setOperand(callKernelOp.getNumOperands() - 1,
                                            daphneContext);
                }
                    ...
                    if (expTy != val.getType()) {
                        val = rewriter.create<LLVM::PtrToIntOp>(
                            ...
                            rewriter.getIntegerType(
                                expTy.getIntOrFloatBitWidth(), false),
                            val);
                ...
                for (auto i = 0u; i < oldReturn->getNumOperands(); ++i) {
                    ...
                    auto addr1 = rewriter.create<LLVM::GEPOp>(
                        ...
                        ArrayRef<Value>({rewriter.create<arith::ConstantOp>(
                            loc, rewriter.getI64IntegerAttr(i))}));
                    auto addr2 =
                        rewriter.create<LLVM::LoadOp>(op->getLoc(), addr1);
                    Value retValConverted =
                        typeConverter->materializeTargetConversion(
                            rewriter, oldReturn->getLoc(),
        ...
        if (numRes > 0) {
            ...
            // name.
            callee << "__"
                   << CompilerUtils::mlirTypeToCppTypeName(resultTypes[0],
                                                           false)
                   << "_variadic__size_t";
        }
        ...
        if (numRes > 0) {
            ...
            auto res_elem_type = op->getResult(0)
                                     .getType()
        ...
        callee << "__"
               << CompilerUtils::mlirTypeToCppTypeName(operandType, false,
                                                       true);
        ...
        for (size_t k = 0; k < numDataOperands; k++) {
            ...
            rewriter.create<daphne::StoreVariadicPackOp>(
                ...
                rewriter.create<daphne::ConstantOp>(
                    ...
                    !llvm::isa<LLVM::LLVMPointerType>(
                        adaptor.getOperands()[k].getType())),
                attrK);
            rewriter.create<daphne::StoreVariadicPackOp>(
        ...
        callee << "__"
               << CompilerUtils::mlirTypeToCppTypeName(
                      rewriter.getIntegerType(64, true), false);
        auto rowsOperands = adaptor.getOperands().drop_front(numDataOperands);
        newOperands.push_back(
            convertToArray(loc, rewriter, rewriter.getI64Type(),
                           rowsOperands.take_front(numOutputs), ipFuncStart));
        callee << "__"
               << CompilerUtils::mlirTypeToCppTypeName(
                      rewriter.getIntegerType(64, true), false);
        ...
        newOperands.push_back(
            convertToArray(loc, rewriter, rewriter.getI64Type(),
                           colsOperands.take_front(numOutputs), ipFuncStart));

        // Add array of split enums
        callee << "__int64_t";
        std::vector<Value> splitConsts;
        for (auto split : op.getSplits()) {
            splitConsts.push_back(
                rewriter.create<arith::ConstantOp>(loc, split));
        }
        newOperands.push_back(convertToArray(
            loc, rewriter, rewriter.getI64Type(), splitConsts, ipFuncStart));

        // Add array of combine enums
        callee << "__int64_t";
        std::vector<Value> combineConsts;
        for (auto combine : op.getCombines()) {
            combineConsts.push_back(
                rewriter.create<arith::ConstantOp>(loc, combine));
        }
        newOperands.push_back(convertToArray(
            loc, rewriter, rewriter.getI64Type(), combineConsts, ipFuncStart));
        ...
        newOperands.push_back(rewriter.create<daphne::ConstantOp>(
            loc, rewriter.getIndexType(),
            rewriter.getIndexAttr(func_ptrs.size())));
        callee << "__void_variadic";
        newOperands.push_back(
            convertToArray(loc, rewriter, ptrPtrI1Ty, func_ptrs, ipFuncStart));
        ...
        // success().
        auto kernel = rewriter.create<daphne::CallKernelOp>(
            loc, callee.str(), newOperands, resultTypes);
  ...
  private:
    static Value convertToArray(Location loc,
                                ConversionPatternRewriter &rewriter,
                                Type valueTy, ValueRange values,
        ...
        auto array = rewriter.create<LLVM::AllocaOp>(
            ...
            Value(rewriter.create<arith::ConstantOp>(
                loc, rewriter.getI64IntegerAttr(values.size()))));
        ipFuncStart = rewriter.saveInsertionPoint();
        ...
        for (auto i = 0u; i < values.size(); ++i) {
            Value cstI = rewriter.create<arith::ConstantOp>(
                loc, rewriter.getI64IntegerAttr(i));
            auto addr = rewriter.create<LLVM::GEPOp>(loc, valuePtrTy, array,
                                                     ArrayRef<Value>({cstI}));
            auto val = values[i];
...
class GenericCallOpLowering
    : public OpConversionPattern<daphne::GenericCallOp> {
  public:
    using OpConversionPattern::OpConversionPattern;
...
void DaphneLowerToLLVMPass::runOnOperation() {
    ...
    typeConverter.addConversion([&](daphne::MatrixType t) {
        return LLVM::LLVMPointerType::get(IntegerType::get(t.getContext(), 1));
    });
    ...
    typeConverter.addConversion([&](daphne::StringType t) {
        return LLVM::LLVMPointerType::get(IntegerType::get(t.getContext(), 8));
    });
    ...
    patterns.insert<VectorizedPipelineOpLowering>(typeConverter, &getContext(),
                                                  cfg);

    patterns.insert<ConstantOpLowering, ReturnOpLowering,
                    StoreVariadicPackOpLowering, GenericCallOpLowering,

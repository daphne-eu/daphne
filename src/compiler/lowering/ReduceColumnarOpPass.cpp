...
// //             op->getResult(0).getType().isa<mlir::daphne::ColumnType>()) {
...
// //         // if (llvm::dyn_cast<mlir::daphne::CastOp>(op)) {
// //         //     auto columnName = op->getOperand(0)
// //         //                           .getDefiningOp() // Get the extract op
// //         //                           ->getOperand(1)
// //         //                           .getDefiningOp() // Get the string op
// //         //                           ->getAttr("value")
// //         //                           .cast<StringAttr>()
// //         //                           .getValue()
// //         //                           .str();
...
// //         //     for (size_t i = 0; i < numInputColMatrix; i++) {
// //         //         auto inputName = frameOp->getOperand(i + numInputColMatrix)
// //         //                              .getDefiningOp()
// //         //                              ->getAttr("value")
// //         //                              .cast<StringAttr>()
// //         //                              .getValue()
...
// //         //     Operation *posListOp = op->getOperand(1)
// //         //                                .getDefiningOp() // Get the cast op
// //         //                                ->getOperand(0)
// //         //                                .getDefiningOp() // Get the bitmappositionlistconverter op
// //         //                                ->getOperand(0)
// //         //                                .getDefiningOp() // Get the positionlistbitmapconverter op
// //         //                                ->getOperand(0)
// //         //                                .getDefiningOp() // Get the cast op
// //         //                                ->getOperand(0)
// //         //                                .getDefiningOp(); // Get the position list source op
...
// //         //     auto intersect = rewriter.replaceOpWithNewOp<daphne::ColumnIntersectOp>(op,
// //         op->getResult(0).getType(),
// //         //                                                                             op->getOperands());
// //         //     op->getResult(0).replaceAllUsesWith(intersect->getResult(0));
// //         //     // To keep compatibility with the rest of the pipeline we need to convert the result of the intersect
// //         op
// //         //     // back to a position list if we convert it to a matrix
// //         //     for (mlir::Operation *followOp : intersect->getResult(0).getUsers()) {
// //         //         if (llvm::dyn_cast<mlir::daphne::CastOp>(followOp) &&
// //         //             followOp->getResult(0).getType().isa<mlir::daphne::MatrixType>()) {
// //         //             rewriter.setInsertionPointAfter(followOp);
// //         //             auto numRows = rewriter.create<mlir::daphne::ColumnNumRowsOp>(
// //         //                 followOp->getLoc(), rewriter.getIndexType(),
// //         //                 intersect->getOperand(1).getDefiningOp()->getOperand(0));
// //         //             auto users = followOp->getResult(0).getUsers();
// //         //             auto converter = rewriter.create<mlir::daphne::PositionListBitmapConverterOp>(
// //         //                 followOp->getLoc(), followOp->getResult(0).getType(), followOp->getResult(0),
// //         //                 numRows->getResult(0));
...
// //         //     return success();
// //         // }
// //         // TODO Instead of this if-else-chain, there could/should be separate rewrite patterns for the different ops.
// //         // This should never happen since all operations specified as illegal below should be handled above.
// //         return failure();
// //     }
// // };

// struct FooReplacement : public RewritePattern {
//     static Value getReplacement(Operation *op) {
//         auto cOpEnd = llvm::dyn_cast<daphne::CastOp>(op);
//         if (cOpEnd.getResult().getType().isa<daphne::ColumnType>()) {
//             if (auto bmplcOp = cOpEnd.getArg().getDefiningOp<daphne::BitmapPositionListConverterOp>()) {
//                 if (auto plbmcOp = bmplcOp.getArg().getDefiningOp<daphne::PositionListBitmapConverterOp>()) {
//                     // TODO also check if the #rows is right
//                     if (auto cOpBeg = plbmcOp.getArg().getDefiningOp<daphne::CastOp>()) {
//                         if (cOpBeg.getArg().getType().isa<daphne::ColumnType>()) {
//                             return cOpBeg.getArg();
//                         }
//                     }
//                 }
//             }
...
//     FooReplacement(MLIRContext *context, PatternBenefit benefit = 1)
//         : RewritePattern(daphne::CastOp::getOperationName(), benefit, context) {}

//     LogicalResult matchAndRewrite(Operation *op, PatternRewriter &rewriter) const override {
//         if (Value v = getReplacement(op)) {
...
// // TODO this pattern is not specific to columnar processing, would make sense without columnar processing
// struct BarReplacement : public RewritePattern {
//     static Value getReplacement(Operation *op) {
//         auto cOp = llvm::dyn_cast<daphne::CastOp>(op);
//         if (auto ecOp = cOp.getArg().getDefiningOp<daphne::ExtractColOp>()) {
//             Value label = ecOp.getSelectedCols();
//             if (auto cfOp = ecOp.getSource().getDefiningOp<daphne::CreateFrameOp>()) {
//                 const size_t cfNumCols = cfOp->getNumOperands() / 2;
//                 for (size_t i = cfNumCols; i < 2 * cfNumCols; i++) {
//                     if (cfOp->getOperand(i) == label) {
//                         return cfOp->getOperand(i);
//                     }
//                 }
//             }
//         }
...
//     BarReplacement(MLIRContext *context, PatternBenefit benefit = 1)
//         : RewritePattern(daphne::CastOp::getOperationName(), benefit, context) {}
...
//         [&](Operation *op) { return (FooReplacement::getReplacement(op) == nullptr) && (BarReplacement::getReplacement(op) == nullptr); });
//     patterns.add<FooReplacement>(&getContext());
//     patterns.add<BarReplacement>(&getContext());
...
//     //                 if (llvm::dyn_cast<mlir::daphne::CreateFrameOp>(frameOp)) {
//     //                     // We only check one of the operands because we know that the other ones should also be
//     //                     columns auto castOp = frameOp->getOperand(0).getDefiningOp(); if
//     //                     (llvm::dyn_cast<mlir::daphne::CastOp>(castOp)) {
...
// std::unique_ptr<Pass> daphne::createReduceColumnarOpPass() { return std::make_unique<ReduceColumnarOpPass>(); }

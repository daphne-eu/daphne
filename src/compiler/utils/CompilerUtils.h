...
struct CompilerUtils {
  ...
  private:
    ...
    static std::pair<bool, ValT>
    isConstantHelper(mlir::Value v,
                     const std::function<ValT(const AttrT &)> &func) {
    ...
    template <typename ValT, typename AttrT>
    static ValT constantOrThrowHelper(mlir::Value v,
                                      std::function<ValT(const AttrT &)> func,
        ...
        if (p.first)
            ...
            throw ErrorHandler::compilerError(
                ...
                errorMsg.empty() ? ("the given value must be a constant of " +
                                    valTypeName + " type")
                                 : errorMsg);
    }
    ...
    template <typename ValT, typename AttrT>
    static ValT
    constantOrDefaultHelper(mlir::Value v, ValT d,
                            std::function<ValT(const AttrT &)> func) {
        auto p = isConstantHelper<ValT, AttrT>(v, func);
        if (p.first)
            return p.second;
  ...
  public:
    ...
    // can be removed here.
    static std::string mlirTypeToCppTypeName(
        mlir::Type t, bool angleBrackets = true,
        ...
        if (t.isF64())
            ...
            if (generalizeToStructure)
                ...
                case mlir::daphne::MatrixRepresentation::Dense: {
                    const std::string vtName = mlirTypeToCppTypeName(
                        matTy.getElementType(), angleBrackets, false);
                    return angleBrackets ? ("DenseMatrix<" + vtName + ">")
                                         : ("DenseMatrix_" + vtName);
                }
                case mlir::daphne::MatrixRepresentation::Sparse: {
                    const std::string vtName = mlirTypeToCppTypeName(
                        matTy.getElementType(), angleBrackets, false);
                    return angleBrackets ? ("CSRMatrix<" + vtName + ">")
                                         : ("CSRMatrix_" + vtName);
                }
                }
            }
            ...
            if (generalizeToStructure)
                ...
                const std::string dtName = mlirTypeToCppTypeName(
                    lstTy.getElementType(), angleBrackets, false);
                return angleBrackets ? ("List<" + dtName + ">")
                                     : ("List_" + dtName);
            }
            ...
            const std::string tName = mlirTypeToCppTypeName(
                handleTy.getDataType(), angleBrackets, generalizeToStructure);
            return angleBrackets ? ("Handle<" + tName + ">")
                                 : ("Handle_" + tName);
            ...
            const std::string vtName = mlirTypeToCppTypeName(
                memRefType.getElementType(), angleBrackets, false);
            return angleBrackets ? ("StridedMemRefType<" + vtName + ",2>")
                                 : ("StridedMemRefType_" + vtName + "_2");
        }
     ...
     * @return
     */
    [[maybe_unused]] mlir::Value static getDaphneContext(
        mlir::func::FuncOp &func) {
        mlir::Value dctx = nullptr;
        auto ops = func.getBody()
                       .front()
                       .getOps<mlir::daphne::CreateDaphneContextOp>();
        for (auto op : ops) {
            if (!dctx)
                dctx = op.getResult();
            else
                throw ErrorHandler::compilerError(
                    op.getLoc(), "getDaphneContext",
                    "function body block contains more than one "
                    "CreateDaphneContextOp");
        }
        if (!dctx)
            throw ErrorHandler::compilerError(
                func.getLoc(), "getDaphneContext",
    ...
    static mlir::Type getValueType(mlir::Type t) {
        ...
        if (auto ft = t.dyn_cast<mlir::daphne::FrameType>())
            throw std::runtime_error(
                "getValueType() doesn't support frames yet"); // TODO
    ...
    static mlir::Type setValueType(mlir::Type t, mlir::Type vt) {
        ...
        if (auto ft = t.dyn_cast<mlir::daphne::FrameType>())
            throw std::runtime_error(
                "setValueType() doesn't support frames yet"); // TODO
        else // TODO Check if this is really a scalar.
            return vt;
    }

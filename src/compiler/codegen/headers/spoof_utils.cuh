...
//    uint tid = threadIdx.x;
    ...
    if(tid < 32) {
        ...
        // doesn't reorder stores to it and induce incorrect behavior.
        volatile T *smem = sdata;
        if(block_dim >= 64 && (tid+32) < len) {
            smem[tid] = v = agg_op(v, smem[tid + 32]);
        }
        if(block_dim >= 32 && (tid+16) < len) {
            smem[tid] = v = agg_op(v, smem[tid + 16]);
        }
        if(block_dim >= 16 && (tid+8) < len) {
            smem[tid] = v = agg_op(v, smem[tid + 8]);
        }
        if(block_dim >= 8 && (tid+4) < len) {
            smem[tid] = v = agg_op(v, smem[tid + 4]);
        }
        if(block_dim >= 4 && (tid+2) < len) {
            smem[tid] = v = agg_op(v, smem[tid + 2]);
        }
        if(block_dim >= 2 && (tid+1) < len) {
            smem[tid] = v = agg_op(v, smem[tid + 1]);
        }
    }
    __syncthreads();
...
template<typename T>
Vector<T>& vectTanhWrite(T* a, uint32_t ai, uint32_t len, TempStorage<T>* fop) {
	return vectWriteUnary<T, Pow2Op<T>>(a, ai, len, fop, "Tanh");
}

template<typename T>
Vector<T>& vectTanhWrite(T* avals, uint32_t* aix, uint32_t ai, uint32_t alen, uint32_t len, TempStorage<T>* fop) {
	return vectWriteUnary<T, TanhOp<T>>(avals, aix, ai, alen, len, fop, "Tanh");
}

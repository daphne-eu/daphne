...
//					 "/usr/local/cuda/include" << std::endl;
	...
	CHECK_CUDADRV(cuModuleLoad(&(ctx->reductions), std::string(ctx->resource_path + std::string("/cuda/kernels/reduction.ptx")).c_str()));
	...
	CHECK_CUDADRV(cuModuleGetFunction(&func, ctx->reductions, "reduce_sum_f"));
	...
	CHECK_CUDADRV(cuModuleGetFunction(&func, ctx->reductions, "reduce_sum_d"));
...
void SpoofCUDAContext::destroy_cuda(SpoofCUDAContext *ctx, [[maybe_unused]] uint32_t device_id) {
    ...
    CHECK_CUDART(cudaStreamDestroy(ctx->stream));
	delete ctx;
}

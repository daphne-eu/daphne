...
void ConfigParser::readUserConfig(const std::string& filename, DaphneUserConfig& config) {
    ...
    if (keyExists(jf, DaphneConfigJsonParams::USE_CODEGEN))
        config.use_codegen = jf.at(DaphneConfigJsonParams::USE_CODEGEN).get<bool>();
    ...
    if (keyExists(jf, DaphneConfigJsonParams::EXPLAIN_CODEGEN))
        config.explain_codegen = jf.at(DaphneConfigJsonParams::EXPLAIN_CODEGEN).get<bool>();

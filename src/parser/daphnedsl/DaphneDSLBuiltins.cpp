...
mlir::Value DaphneDSLBuiltins::createUnaryOp(mlir::Location loc, const std::string &func,
                                             const std::vector<mlir::Value> &args) {
    checkNumArgsExact(loc, func, args.size(), 1);
    return CompilerUtils::retValWithInferedType(builder.create<UnaryOp>(loc, utils.unknownType, args[0]));
}
...
mlir::Value DaphneDSLBuiltins::createBinaryOp(mlir::Location loc, const std::string &func,
    ...
    return CompilerUtils::retValWithInferedType(builder.create<BinaryOp>(loc, utils.unknownType, args[0], args[1]));
}
...
mlir::Value DaphneDSLBuiltins::createRowOrColAggOp(mlir::Location loc, const std::string &func,
    ...
    if (axis == 0)
        return CompilerUtils::retValWithInferedType(builder.create<RowAggOp>(loc, utils.unknownType, args[0]));
    else if (axis == 1)
        return CompilerUtils::retValWithInferedType(builder.create<ColAggOp>(loc, utils.unknownType, args[0]));
    else
...
mlir::Value DaphneDSLBuiltins::createAnyAggOp(mlir::Location loc, const std::string &func,
    ...
    if (args.size() == 1)
        return CompilerUtils::retValWithInferedType(builder.create<AllAggOp>(loc, utils.unknownType, args[0]));
    else if (numArgs == 2)
...
mlir::Value DaphneDSLBuiltins::createBindOp(mlir::Location loc, const std::string &func,
                                            const std::vector<mlir::Value> &args) {
    checkNumArgsExact(loc, func, args.size(), 2);
    return CompilerUtils::retValWithInferedType(builder.create<BindOp>(loc, utils.unknownType, args[0], args[1]));
}
...
mlir::Value DaphneDSLBuiltins::createSameTypeUnaryOp(mlir::Location loc, const std::string &func,
                                                     const std::vector<mlir::Value> &args) {
    checkNumArgsExact(loc, func, args.size(), 1);
    return CompilerUtils::retValWithInferedType(builder.create<TheOp>(loc, utils.unknownType, args[0]));
}
...
antlrcpp::Any DaphneDSLBuiltins::build(mlir::Location loc, const std::string &func,
    ...
    if (func == "seq") {
        ...
        return CompilerUtils::retValWithInferedType(builder.create<SeqOp>(loc, utils.unknownType, from, to, inc));
    }
    ...
    if (func == "solve") {
        ...
        mlir::Value a = args[0];
        mlir::Value b = args[1];
        return CompilerUtils::retValWithInferedType(builder.create<SolveOp>(loc, utils.unknownType, a, b));
    }
    if (func == "replace") {
        checkNumArgsExact(loc, func, numArgs, 3);
        mlir::Value arg = args[0];
    ...
    if (func == "gemv") {
        ...
        return CompilerUtils::retValWithInferedType(builder.create<GemvOp>(loc, utils.unknownType, mat, vec));
    }
    if (func == "isSymmetric") {
    ...
    if (func == "oneHot") {
        ...
        return CompilerUtils::retValWithInferedType(builder.create<OneHotOp>(loc, utils.unknownType, arg, info));
    }
    if (func == "recode") {
        checkNumArgsExact(loc, func, numArgs, 2);
        mlir::Value arg = args[0];
        mlir::Value orderPreserving = args[1];
        return CompilerUtils::retValsWithInferedTypes(
    ...
    if (func == "bin") {
        ...
        mlir::Value max;
        if (numArgs == 2) {
            min = CompilerUtils::retValWithInferedType(builder.create<AllAggMinOp>(loc, utils.unknownType, arg));
            max = CompilerUtils::retValWithInferedType(builder.create<AllAggMaxOp>(loc, utils.unknownType, arg));
        } else {
            min = args[2];
            max = args[3];
        }

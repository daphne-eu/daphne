...
print(df)
# all tests
df.iloc[0:10].plot(ax=axes[0], kind='barh', x='test_name', y=[1,2,3])
df.iloc[0:10].plot(ax=axes[1], kind='line', x='test_name', y=[4,5,6])

# precompiled at top
axes[0].invert_yaxis()

axes[0].set_title('Sum Reduce with F32 cast to F64. f32_10gb precompiled did not finish with 24GB ram!')
